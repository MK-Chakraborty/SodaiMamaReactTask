  ...
  return (
    <section className="card">
      ...
      <div className="cardText">
        <p className="cardTitle">{product.title.slice(0, 50)}</p>
        <p className="cardsubtitle">each</p>
        <p className="cardPrice">
          ৳ <span>{Math.ceil(product.price * 85)}</span>
        </p>
        <a href="/">
          <p className="detailsButton">Details</p>
        </a>
        <div className="productDetails">
          <div className="deliveryStatus">
            <img
              src="https://shodaimama.com/static/media/fast-delivery.86065e4e.png"
              alt="bike"
            />
            <p>Next Morning</p>
          </div>
      ...
      <div className="cardBtn" onClick={() => handleAddToCart(product)}>
        Add To Cart
      </div>
    </section>
  );
